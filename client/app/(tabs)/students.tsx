...
export default () => {
  ...
  const handleDelete = async(id: string) => {
    await axios.delete(`http://${host}/students/${id}`)
  ...
  const AddMenu = () => {
    return (
      <>
        <Pressable style={addMenuStyle.container}  onPress={() => router.push("/adds/addstudent")}>
    ...
    const rightAction = () => {
      return (
        <>
          ...
          <Pressable style={stylesOptions.modify_btn} onPress={() => router.push(`/edits/editstudent/${item.id}`)}>
            <Ionicons name="create-outline" size={32} color="black" />
            <Text style={stylesOptions.text}>Modificar</Text>
          </Pressable>

          <Pressable style={stylesOptions.enrolled_btn} onPress={() => router.push(`/enroll/${item.id}`)}>
            <Ionicons name="library-outline" size={32} color="black" />
            <Text style={stylesOptions.text}>Matricular</Text>
          </Pressable>
        </>
      )
    }
